...
//use(url, (req,res)=>{}) 형태이다. 맨 앞의 url이 생략될수도 있는데 생략시 모든 url 경로를 다 처리한다. 
//그래서 url 이 없는 미들웨어의 경우에는 맨 마지막에 두어야 한다. 
//이걸 젤 먼저 두면 모든 url을 이 미들웨어(함수?) 가 모든 url에 대한 처리를 다 하기때문에 
//다른 미들웨어가 처리를 못한다.  폭포수모형 그래서 꼭 젤 마지막에 url이 없는 미들웨어를 두어야 한다 
...
app.use("/a", (req, res)=>{
        ...
        res.end('<h1>aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</h1>');
    }   
);
...
//http://127.0.0.1:4000/userinfo/Bread/34 
...
//get - query  et2-params  post-body
//http://127.0.0.1:4000/rect
app.post("/rect", (req, res)=>{
    console.log( req.body ); //get2방식
...
app.use((req, res)=>{
    ...
    //사용자가 함부로 막 url을 입력하면 그렇게 하면 안된다라는 메시지를 뿌리기 위해 그리고 로그온 페이지나 
    //홈페이지로 이동을 한다 
