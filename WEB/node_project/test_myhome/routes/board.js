...
router.get('/view/:id', async (req,res,next)=>{
  ...
  sql2 = `
        ...
        LIMIT 10
  `;
  // 위에 같은경우 특수 기호가 있어서 백틱으로 감싸줘야 오류가 안뜸
  await testdb.saveDB(sql2, [id]);
  let board = await db.loadDB(sql, [id]);
  let dbResult = await testdb.loadDB(sql2, [id]);

    // 데이터베이스에서 가져온 값을 변수에 할당
    let 등록구분 = dbResult[0].등록구분;
    let 진행상태 = dbResult[0].진행상태;
    let 구분 = dbResult[0].구분;

  //반환값이 배열형태이다. 첫번째것만 보내야 한다 
  console.log( board );
  res.render("board/board_view", { board: board[0], 등록구분, 진행상태, 구분 });
});
